...
Router.prototype = {
	...
	go : function (controllerName, forceHide) {
		...
		/*for (var i = 0; i < this._routes.length; i++) { // em cada rota
			var route = this._routes[i];
			var rn = route.routeName; // nome da rota
			var rp = route.params; // parametros da rota

			if(rp) { // caso tenha parametros
				for (var property in this._routeParams) { // em cada routeparam
					if(property == rn){ // 
						realRoute = rn;
						var test = controllerName.split(rn);

						if(test.length > 1) { // caso tenha parametros
							var urlParams = test[1].split('/'); 
							var keys = Object.keys( this._routeParams[property] );

							for (var i = 1; i < urlParams.length; i++) {
								this._routeParams[property][keys[i-1]] = urlParams[i];
							}
						}
					}
				}
